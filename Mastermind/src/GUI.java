...
public class GUI {
	public static void main (String [] args){						//Spacing
		...
		boolean boolvar = user_input.next().equals("admin");		//"Cheat" um die Lösung anzuzeigen
		
		for (int i = 0; i<10; i++){									//Anzeigen der Lösung, wenn admin eigegeben
			if (boolvar){
				for ( int j = 0; j<4; j++){
			...
			for (int i1 = 0; i1<4; i1++){
				if(logik.tempRichtig==4){							//Wenn gewonnen: Üerspringt anzeigen der Lösung
					break;
				}
